...
pub extern "C" fn init(args: *mut c_void) -> c_int {
    ...
    let mut log_file = OpenOptions::new().create(true).write(true).open(path).unwrap();
	...
	unsafe {
	    ...
	    while write(args.rdy_pipe_write_fd, ret_ptr as *const c_void, size_of_val(&ret)) == -1 &&
    ...
    log_file.write_all(b"container successfully created\n").unwrap();
